      ...
      >
        <nav className="max-w-7xl mx-auto flex items-center justify-between bg-black/60 backdrop-blur-xl border border-white/10 rounded-full px-6 py-3 shadow-2xl">
          ...
          <div className="hidden md:flex flex-1 max-w-md mx-8">
            <form onSubmit={handleSearch} className="w-full relative">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-full py-2 pl-4 pr-10 focus:outline-none focus:border-[#D4AF37] transition-all text-sm text-white"
              />
          ...
          <div className="hidden md:flex items-center gap-6">
                  ...
                  >
                    ...
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center gap-3">
                        <div className="bg-white/5 border border-white/5 p-2 rounded-lg text-[#D4AF37]"><Dumbbell className="w-4 h-4" /></div>
                        <span className="text-white text-sm font-medium">Free Weights & Plates</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="bg-white/5 border border-white/5 p-2 rounded-lg text-[#D4AF37]"><Activity className="w-4 h-4" /></div>
                        <span className="text-white text-sm font-medium">High-End Cardio Gear</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="bg-white/5 border border-white/5 p-2 rounded-lg text-[#D4AF37]"><ShieldCheck className="w-4 h-4" /></div>
                    ...
                    <Link 
                      ...
                      className="w-full bg-white/5 hover:bg-[#D4AF37] text-white py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-colors group"
                    >
            ...
            <Link to="/wishlist" className="text-zinc-400 hover:text-[#D4AF37] transition-colors relative group">
              <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                <Heart className="h-5 w-5" />
              </motion.div>
              {wishlistItems.length > 0 && (
                <span className="absolute -top-1.5 -right-1.5 bg-[#D4AF37] text-white text-[10px] font-bold h-4 w-4 rounded-full flex items-center justify-center">
                  {wishlistItems.length}
                </span>
              )}
            </Link>
            
            <Link to="/cart" className="text-zinc-400 hover:text-[#D4AF37] transition-colors relative group">
              <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
          ...
          <div className="md:hidden max-w-7xl mx-auto mt-4 bg-black/90 backdrop-blur-xl border border-white/10 rounded-2xl px-4 py-4 space-y-4">
            <form onSubmit={handleSearch} className="relative">
              <input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-4 pr-10 focus:outline-none focus:border-[#D4AF37] text-sm text-white transition-colors"
              />
            ...
            <div className="flex flex-col space-y-3 pt-2">
              ...
              <Link to="/wishlist" onClick={() => setIsMenuOpen(false)} className="text-zinc-400 hover:text-white font-medium flex items-center justify-between">
                Wishlist
                {wishlistItems.length > 0 && <span className="bg-[#D4AF37] text-white text-xs px-2 py-0.5 rounded-full">{wishlistItems.length}</span>}
              </Link>
